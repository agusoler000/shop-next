...
import { ProductImage as ProductImagePrima } from "@prisma/client";
import clsx from "clsx";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";

interface Props {
  product: Partial<Product> & { ProductImage?: ProductImagePrima[] };
...
export const ProductForm = ({ product,categories }: Props) => {
  ...
  async function onSubmit(data:FormInputs){
    ...
    const {ok, product:resultproduct} = await createUpdateProduct(formData);
    if(!ok){
      alert('Error GRAVISIMO')
      return}

      router.replace('/admin/product/'+resultproduct?.slug)
  ...
  function onSizeChanged(size: string) {
    console.log(size);
      ...
      <div className="w-full">
        ...
        <div className="flex flex-col mb-2">
          ...
          <select className="p-2 border rounded-md bg-gray-200"  {...register('categoryId', {required:true})}>
            <option value="">[Seleccione]</option>
      {categories.map((category: ProductCategory) => ( <option key={category.id} value={category.id}>{category.name}</option>))     }
          </select>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {
              ...
              product.ProductImage?.map( (img: ProductImagePrima) => ( 
                <div key={img.id} className="">
                  <ProductImage alt={product.title!} src={img.url} width={300} height={300}  className="rounded-t shadow-md w-full"/>
                  <button 
                  onClick={() => deleteProductImage(img.id,img.url)}
                  type="button" 
                  className="btn-danger rounded-b-xl w-full"> Eliminar </button>
                  </div>
              ))
            }
